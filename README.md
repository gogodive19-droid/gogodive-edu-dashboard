# 고고다이브 강습 현황 대시보드

예약 시스템에서 내려받은 "통계분석_그룹수업" 엑셀을 강사별 · 수업종류별 · 강습장별로 월 단위 분석하는 대시보드입니다.

- 대시보드: https://gogodive19-droid.github.io/gogodive-edu-dashboard/ (비밀번호는 노션 허브 페이지에)
- 설계 문서: [docs/superpowers/specs/2026-09-05-edu-dashboard-design.md](docs/superpowers/specs/2026-09-05-edu-dashboard-design.md)

## 매달 자료 반영하는 법

1. 예약 시스템에서 그 달의 "통계분석 > 그룹수업" 엑셀 3개를 내려받습니다.
2. 파일을 `data/raw/` 폴더에 넣습니다. 이름은 자유이지만 `2026-09_1.xlsx` 처럼 월을 붙이면 찾기 쉽습니다.
   이미 있는 파일은 지우지 마세요. 전체 기간 파일이 함께 있어야 합니다.
3. PowerShell에서 아래를 실행합니다. 비밀번호를 물어보면 대시보드 비밀번호를 입력합니다.

```powershell
.\publish.ps1
```

이 명령 하나가 검사 → 집계 → 암호화 → 깃허브 업로드까지 합니다. 1~2분 뒤 대시보드에 새 달이 보입니다.

Claude Code를 쓰는 경우에는 엑셀 3개를 건네며 "9월 자료 반영해줘"라고만 하면 됩니다.

### 새로운 수업명이 생겼을 때

빌드가 이런 메시지와 함께 멈추면 규칙에 없는 수업명이 생긴 것입니다.

```
!! 규칙에 없는 수업명 1종 — build/mapping.py 에 규칙을 추가하세요:
       3  (프리다이빙 특강) 잠실 종합운동장
```

`build/mapping.py` 의 `CLASS_RULES` 나 `VENUE_KEYS` 에 한 줄을 추가하면 됩니다. 어떤 대분류로 넣을지만 정하면 되고, 나머지는 Claude에게 맡겨도 됩니다.

## 코칭반 탭

코칭반(수당이 나가는 연습 수업)과 자율연습반(수당 없음)이 초급1강 수료 인원에 비해 얼마나 열리고 채워지는지 보는 탭입니다.

- 월별 흐름: 코칭반 횟수·인원 → 자율연습반 횟수·인원 → 초급1강 횟수·인원 → 공급 포화도 두 가지
- 공급 포화도(횟수 기준) = (코칭반 횟수 + 자율연습반 횟수) ÷ 전달 초급1강 참가 인원 × 100
- 공급 포화도(인원 기준) = (코칭반 인원 + 자율연습반 인원) ÷ 전달 초급1강 참가 인원 × 100
- 공급 포화도가 높으면 연습 자리가 넉넉해 신청이 수월할 가능성이 높고, 낮으면 자리가 부족해 불만이 생길 가능성이 높습니다. 코칭반은 수당이 나가므로 평균 인원과 함께 봅니다.
- 일반 코칭반·자율연습반만 씁니다. 딥풀 코칭반·딥풀 자율연습반은 넣지 않습니다. 초급1강은 세부 "초급 1강"만입니다.
- 기간을 여러 달로 잡으면 분자는 그 기간 합계, 분모는 각 달의 전달 인원 합계입니다.
- 포화도 두 판은 증감이 잘 보이도록 세로축을 값 범위에 맞춰 0부터 시작하지 않으며, 점선은 전체 기간 평균입니다.
- 아래에 월별 숫자 표, 년별 숫자 표, 선택한 기간의 강습장별 포화도 표가 있습니다. 월별·년별·강습장별 표는 부문마다 횟수·인원·평균 인원(수업 1회당 참가 인원)을 보여줍니다.
- 년별 포화도는 전달 초급1강 인원이 있는 달만 더해 계산합니다. 2024년은 2월부터입니다.
- 참고: 코칭반 표기는 2025년 5월부터 등장합니다. 그 이전의 "연습반"은 자율연습반으로 봅니다(사장님 확인).

## 딥풀 코칭반 탭

코칭반 탭과 같은 구성이며 대상만 다릅니다.

- 코칭반 → 딥풀 코칭반, 자율연습반 → 딥풀 자율연습반
- 공급 포화도 분모: 전달 (중급 1강 + 마스터 1-2강) 참가 인원. 데이터에 "마스터 1강"이 따로 없고 첫 마스터 수업이 "마스터 1-2강"으로 묶여 있어 그것을 씁니다.
- 분모가 한 달 30~80명으로 작아 월별 값이 크게 흔들릴 수 있습니다. 상단에서 기간을 2~3개월로 잡으면 안정적입니다.
- 일반 코칭반·자율연습반은 넣지 않습니다.

두 탭은 같은 코드를 쓰고 `src/template.html`의 `PRACTICE` 설정(어떤 수업을 코칭반·자율연습반·기준 수업으로 볼지)만 다릅니다.

## 숫자 규칙

| 항목 | 규칙 |
|---|---|
| 인원 | `출석` 컬럼. 결석은 뺍니다 |
| 0명 수업 | 횟수·인원에 넣지 않고 "0명 개설" 참고 숫자로만 보여줍니다 |
| 중복 | 같은 수업일시·강사·수업명이 여러 줄이면 출석이 가장 적은 한 줄만 셉니다 |
| 프리다이빙 이론 [LIVE] | 제외 |
| 인원 구간 | 1~10명 각각, 11명 이상은 한 칸 |
| 수업종류 | 대분류 14개(초급·중급·마스터·자율연습반·딥풀 자율연습반·코칭반·딥풀 코칭반·체험·머메이드·트레이닝·스쿠버·키즈·모두·대학생), 세부 48개 |
| 딥풀 구분 | 딥풀 상품 파일(두 번째 엑셀)의 표기 `(용인 딥스) 코칭반` `(시흥 파라) 중급/마스터 자율연습반`처럼 강습장이 앞에 오는 코칭반·자율연습반은 "딥풀 코칭반" "딥풀 자율연습반"으로 따로 셉니다. `(코칭반) 시흥 파라다이브` `(자율연습반) 용인 딥스테이션`처럼 수업종류가 앞에 오는 초급 상품 표기는 일반 코칭반·자율연습반입니다 |
| 연습반 | 연습반과 자율연습반은 같은 수업으로 보고 "자율연습반"으로 합칩니다. 스쿠버 연습반은 스쿠버, 딥 트레이닝 연습반은 트레이닝에 둡니다 |
| 강습장 | 21곳. 약칭·오타(용인 딥스, 용입 딥스, 시흥 파라, K26, 부천 MS 등)는 정식 이름으로 합칩니다 |

## 폴더

```
data/raw/        원본 엑셀 (깃허브에 올라가지 않음)
build/mapping.py 수업명 → 수업종류·강습장 규칙
build/build.py   엑셀 → dist/index.html
build/test_mapping.py  규칙 검사  (python build/test_mapping.py)
src/template.html      화면
dist/            평문 결과 (깃허브에 올라가지 않음)
index.html       암호화된 결과 — 깃허브 페이지가 보여주는 파일
publish.ps1      검사 → 집계 → 암호화 → 업로드
```

## 직접 단계별로 하고 싶을 때

```powershell
python build/test_mapping.py      # 규칙 검사
python build/build.py             # dist/index.html 생성
$env:STATICRYPT_PASSWORD = "비밀번호"
npx -y staticrypt@3.5.4 dist/index.html -d . --short --remember 30 --config false
git add -A; git commit -m "데이터 2026-09 까지 반영"; git push
```
